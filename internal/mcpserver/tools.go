...
func Register(srv *mcp.Server, eng *broker.Engine, callerFn CallerFunc) {
	...
	mcp.AddTool(srv, &mcp.Tool{
		...
		Description: "Open a persistent SSH session that reuses the connection across commands. " +
			...
			"Every ssh_session_exec is preflighted against the current signer policy, so policy reloads revalidate host access, end-user groups, sudo, sudo_user, PTY, and the host's physical route for already-open sessions. " +
	...
	mcp.AddTool(srv, &mcp.Tool{
		...
		Description: "Execute a command in a session opened with ssh_session_open. " +
			...
			"The command is preflighted against the current signer policy before execution; host access, end-user groups, sudo, sudo_user, PTY, and the host's physical route are revalidated, and audit-mode policy warnings are returned in warnings. " +
